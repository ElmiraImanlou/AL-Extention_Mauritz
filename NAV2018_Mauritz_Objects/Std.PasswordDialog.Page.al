Page 79815 "Std. Password Dialog"
{
    ...
    layout
    {
        area(content)
        {
            field(SetPassword; SetPassword)
            {
                ...
                trigger OnValidate()
                begin
                    if ValidatePassword and not IdentityManagement.IsUserPasswordSet(SetPassword) then
    ...
    begin
        ...
        if CloseAction = Action::OK then begin
            ...
            if EnableBlankPasswordState and (SetPassword = '') then begin
                ...
                if ValidatePassword and not IdentityManagement.IsUserPasswordSet(SetPassword) then //ValidatePasswordStrength(SetPassword)
                    Error(PasswordTooSimpleErr);
            end;
            ValidPassword := true;
        end
