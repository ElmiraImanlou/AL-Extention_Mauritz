Report 50088 "Create XML-File VAT Adv.Notif."
{
    ...
    begin
        ...
        XMLDOMMgt.LoadXMLNodeFromText(
          '<?xml version="1.0" encoding="UTF-8"?>' + '<Elster xmlns="' + xmlNameSpace + '"></Elster>', XMLNodeCurr);

        XMLSubDoc := XMLNodeCurr.OwnerDocument;
    ...
    begin
        ...
        Version :=
          CopyStr(
            'Navision ' + ApplicationMgt.ApplicationVersion + ' Build # ' + ApplicationMgt.ApplicationBuild,
            1, MaxStrLen(Version));
    ...
    begin
        ...
        if XMLDOMMgt.AddElement(XMLNodeCurr, 'ProduktName', 'Microsoft Business Solutions-Navision', xmlNameSpace, XMLNewNode) > 0 then
            exit;
        if XMLDOMMgt.AddElement(XMLNodeCurr, 'ProduktVersion', GetProductVersion, xmlNameSpace, XMLNewNode) > 0 then
            exit;
        XMLNodeCurr := XMLNodeCurr.ParentNode;
    ...
    var
        ...
        Result: Text[50];
    begin
        ProductVersion := ApplicationManagement.ApplicationVersion;
